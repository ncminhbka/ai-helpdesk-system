...
@tool
def search_fpt_policies_with_scores(query: str) -> str:
    """
    Search FPT policies with relevance scores.
    Returns results with similarity scores for transparency.
    
    Args:
        query: The question or search query about FPT policies
    
    Returns:
        Relevant policy information with sources and scores
    """
    try:
        vectorstore = get_vectorstore()
        ...
        for i, (doc, score) in enumerate(docs_with_scores, 1):
            ...
            results.append(
                f"**[{i}] Nguồn: {source} (Trang {page}) - Độ liên quan: {similarity}%**\n"
                f"{doc.page_content}\n"
            )
